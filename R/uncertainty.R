...
apm_recover_uncertainty <- function(data, mean, n, sd = NULL, se = NULL, cv = NULL, mse = NULL, ci_lower = NULL, ci_upper = NULL, level = 0.95, df = NULL, method = c("auto", "sd", "se", "cv", "mse", "ci"), impute = FALSE, group = NULL) {
  ...
  for (m in allowed) {
    ...
    if (m=="cv" && !is.null(vals$cv)) { ok<-idx[!is.na(vals$cv[idx]) & !is.na(mu[idx]) & mu[idx]!=0]; cvv<-vals$cv; percent<-abs(cvv)>1; cvv[percent]<-cvv[percent]/100; s<-abs(cvv*mu); if(length(ok)) use(ok,s,"derived_from_cv", "CV values with absolute value > 1 were interpreted as percent.") }
